...
args.dev_set = '../data/keyword_extract_file.csv'
...
args.test_set = '../data/keyword_extract_file.csv'
args.test_save = '../data/predict.json'
            ...
            try:
                ...
                if steps % args.skip_steps != 0:
                    ...
                    out = train_exe.run(fetch_list=fetch_list,
                                        return_numpy=False)
                    enc_out, logits, labels, loss_lod, lr_lod = out
                ...
                if nccl2_trainer_id == 0 and steps % args.save_steps == 0:
                    ...
                    model_save_path = os.path.join(save_path, "ner_model_model")
                    client_conf_save_path = os.path.join(save_path, "ner_model_client_conf")
                    serving_io.save_model(
                        model_save_path, client_conf_save_path,
                        {"src_ids": graph_vars["input_save"][0],
                         "sent_ids": graph_vars["input_save"][1],
                         "pos_ids": graph_vars["input_save"][2],
                         "task_ids": graph_vars["input_save"][3],
                         "input_mask": graph_vars["input_save"][4],
                         "labels": graph_vars["input_save"][5],
                         "seq_lens": graph_vars["input_save"][6],
                         "example_index": graph_vars["input_save"][7],
                         "tok_to_orig_start_index": graph_vars["input_save"][8],
                         "tok_to_orig_end_index": graph_vars["input_save"][9]},
                        {"lod_logit": graph_vars["lod_logit"],
                         "lod_label": graph_vars["lod_label"]},
                        train_program)
                    print('enc_out:type', graph_vars["enc_out"], 'saved')
                    for i in range(10):
                        print(graph_vars["input_save"][i])
                    with open('enc_out.text','w') as f_w:
                        f_w.write(enc_out)

                    if nccl2_trainer_id == 0 and args.do_val:
                        evaluate_wrapper(reader, exe, test_prog, test_pyreader, graph_vars, 'final', 'final')
...
def predict_wrapper(reader, exe, test_prog, test_pyreader, graph_vars, epoch,
    ...
    for test_f, save_f in zip(test_sets, save_dirs):
        test_pyreader.decorate_tensor_provider(
            reader.test_data_generator(
                test_f,
                batch_size=batch_size,
                epoch=1,
                dev_count=1,
                shuffle=False))
        examples = reader.get_resourse_data(test_f)
        save_path = save_f
        log.info("testing {}, save to {}".format(test_f, save_path))
        res, pre_time = predict(args, examples, exe, test_prog, test_pyreader, graph_vars)
        print("语义引擎耗时：\t{}秒".format(pre_time))
        with open('time.text', 'w', encoding='utf-8') as f_w:
            f_w.write(str(pre_time))
