...
def create_model(args, pyreader_name, ernie_config):
    ...
    input_save = fluid.layers.read_file(pyreader)
    (src_ids, sent_ids, pos_ids, task_ids, input_mask, labels, seq_lens,
     example_index, tok_to_orig_start_index,
     tok_to_orig_end_index) = input_save

    # src_ids.is_lod_tensor = True
    ...
    for k, v in graph_vars.items():
        if k not in ["input_save"]:
            v.persistable = True
        else:
            for lod_tensor in v:
                # print(lod_tensor.name)
                lod_tensor.persistable = True
...
def calculate_acc(logits, labels):
    ...
    for i in range(len(logits_lod[0]) - 1):
        ...
        '''print("-----start1")
        print(inference_tmp)
        print(inference_tmp.shape)
        for t in range(len(inference_tmp)):
            max_t = inference_tmp[t].max()
            for t1 in range(inference_tmp[t].size):
                if(inference_tmp[t][t1] >= max_t):
                    inference_tmp[t][t1] = 1
                else:
                    inference_tmp[t][t1] = 0'''
    ...
    while True:
        try:
            ...
            for i in range(len(logits_lod[0]) - 1):
                ...
                inference_tmp[inference_tmp >= 0.5] = int(1)
                inference_tmp[inference_tmp < 0.5] = int(0)
                predict_result = []
    ...
    while True:
        try:
            ...
            for i in range(len(logits_lod[0]) - 1):
                ...
                #print(inference_tmp)
                # logits -> classification results
                inference_tmp[inference_tmp >= 0.8] = 1
                inference_tmp[inference_tmp < 0.8] = 0
                predict_result = []
                '''
                f = open('/home/aistudio/DuIe_baseline/data/f1.txt', 'a', encoding='UTF-8')
                f.writelines(str(example['text']))
                f.write('\n')
                for i in inference_tmp:
                    for j in i:
                        f.write(str(j))
                        f.write('/')
                    f.write('\n')
                ...
                formated_result = format_output(
                    example, predict_result, spo_label_map,
                    tok_to_orig_start_index, tok_to_orig_end_index)
                if formated_result['spo_list'] == []:
                    continue
                else:
                    for spo in formated_result['spo_list']:
                        new_spo = {}
                        new_spo['predicate'] = spo['predicate']
                        new_spo['subject'] = spo['subject']
                        new_spo['subject_type'] = spo['subject_type']
                        new_spo['object'] = list(spo['object'].values())[0]
                        new_spo['object_type'] = list(spo['object_type'].values())[0]
                        new_spo['confidence'] = 0

                        try:
                            date = timefinder.find_time(formated_result['text'])
                        except Exception:
                            date = None
                        if date is None:
                            date = 'unknown'

                        if len(new_spo['subject']) < 2 or len(new_spo['object']) < 2:
                            continue

                        new_line = {'spo_list': new_spo, 'text': formated_result['text'], 'source': '', 'url': '', 'date': date}
                        res.append(new_line)
        except fluid.core.EOFException:
            test_pyreader.reset()
            break
...
def format_output(example, predict_result, spo_label_map,
    ...
    for cls_label in list(set(flatten_predict)):
        if 1 < cls_label <= 59 and (cls_label + 58) in flatten_predict:
    ...
    for id_ in subject_id_list:
        if id_  in simple_relation_lable:
            subjects = find_entity(id_, predict_result)
            objects = find_entity(id_ + 58, predict_result)
            ...
            for subject_ in subjects:
                for object_ in objects:
                    object_dict = {}
                    object_type_dict = {}
                    if len(find_entity(id_ + 58, predict_result)[0]) > 0:
                        object_dict[spo_label_map['object_type'][id_].split(
                            '_')[1]] = find_entity(id_ + 58,
                                                    predict_result)[0]
                        object_type_dict[spo_label_map['object_type'][
                            id_].split('_')[1]] = spo_label_map[
                                'object_type'][id_].split('_')[0]
                        spo_list.append({
                            "predicate": spo_label_map['predicate'][id_],
