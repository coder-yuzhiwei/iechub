...
def evaluate_wrapper(reader, exe, test_prog, test_pyreader, graph_vars, epoch,
    ...
    for ds in args.dev_set.split(','):  # single card eval
        ...
        examples, data_len, cost_time = reader.get_resourse_data(ds)
    ...
    for test_f, save_f in zip(test_sets, save_dirs):
        test_pyreader.decorate_tensor_provider(
            reader.test_data_generator(
                ...
                dev_count=1,
                shuffle=False))
        examples, data_len, cost_time = reader.get_resourse_data(test_f)
        save_path = save_f
        ...
        log.info("语义引擎耗时：\t{}秒".format(pre_time))
        with open(str(save_path)+'.txt', 'w', encoding='utf-8') as f_w:
            f_w.write(str(data_len)+' '+str(pre_time+cost_time))
        save_dir = os.path.dirname(save_path)
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        with codecs.open(str(save_path)+'.json', 'w', 'utf-8') as f:
            for result in res:
